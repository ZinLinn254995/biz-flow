...
- Every completed task workflow must end with an automatic push to `main` on GitHub — this is not optional and does not require the human to separately ask for a push. Specifically: after the End-of-Task Report is posted and verification (typecheck/test/build) passes, the AI must, in the same turn, without waiting for further instruction: (1) merge the working branch into main with a normal merge, (2) push main to origin, (3) fetch fresh from GitHub to confirm main's HEAD matches what was just pushed, and (4) include that confirmed main HEAD SHA in the End-of-Task Report itself, not in a separate follow-up message. A task must never be left in a state where local commits or a feature-branch commit exist but main on GitHub has not been updated to match.
