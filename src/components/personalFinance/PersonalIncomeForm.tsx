...
export function PersonalIncomeForm({
  open,
  income,
  categories,
  accounts,
  ...
  useEffect(() => {
    if (open) {
      ...
      setAmount(income ? formatFromMinor(income.amount.amountMinor) : '0');
      setCurrency(income?.amount.currency ?? 'USD');
      setAccountId(income?.accountId ?? '');
      setNotes(income?.notes ?? '');
      setLocalError(null);
      const timer = setTimeout(() => sourceInputRef.current?.focus(), 50);
  ...
  const availableCategories = categories.filter(
    (c) => c.scope === 'personal' && (!c.direction || c.direction === 'income'),
  );
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!trimmedSource) {
      setLocalError('Income source is required.');
      return;
    }
    if (!date.trim()) {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 sticky top-0 bg-white rounded-t-xl">
          <h2 id="income-form-title" className="text-base font-semibold text-gray-900">
        ...
        <form onSubmit={handleSubmit} noValidate className="px-5 py-4 space-y-4">
          <div>
            <label htmlFor="income-source" className="block text-sm font-medium text-gray-700 mb-1.5">
              Source <span className="text-red-500" aria-hidden="true">*</span>
            </label>
            <input
              ref={sourceInputRef}
              id="income-source"
          ...
          <div>
            <label htmlFor="income-category" className="block text-sm font-medium text-gray-700 mb-1.5">
              Category
            </label>
            <select
              id="income-category"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="income-date" className="block text-sm font-medium text-gray-700 mb-1.5">
            ...
            <div>
              <label htmlFor="income-amount" className="block text-sm font-medium text-gray-700 mb-1.5">
          ...
          <div>
            <label htmlFor="income-account" className="block text-sm font-medium text-gray-700 mb-1.5">
              Account
            </label>
            <select
              id="income-account"
          ...
          <div>
            <label htmlFor="income-notes" className="block text-sm font-medium text-gray-700 mb-1.5">
              Notes
            </label>
            <textarea
              id="income-notes"
