...
export class DexieDataBackupRepository implements DataBackupRepository {
  async exportData(): Promise<BackupData['data']> {
    const [businesses, inventoryItems, sales, purchases, customers, businessExpenses, personalIncomes, personalExpenses, categories, budgets, accounts, savedItems] = await Promise.all([
      db.businesses.toArray(), db.inventoryItems.toArray(), db.sales.toArray(), db.purchases.toArray(), db.customers.toArray(),
    ...
    return { businesses, inventoryItems, sales, purchases, customers, businessExpenses, personalIncomes, personalExpenses, categories, budgets, accounts, savedItems };
  }
  ...
  async replaceData(data: BackupData['data']): Promise<void> {
    await db.transaction('rw', [
      ...
      await Promise.all([
        db.businesses.clear(), db.inventoryItems.clear(), db.sales.clear(), db.purchases.clear(), db.customers.clear(),
