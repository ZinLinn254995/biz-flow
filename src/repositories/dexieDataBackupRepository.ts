...
export class DexieDataBackupRepository implements DataBackupRepository {
  async exportData(): Promise<BackupData['data']> {
    const [businesses, inventoryItems, sales, customers, businessExpenses, personalIncomes, personalExpenses, categories, budgets, accounts] = await Promise.all([
      ...
      db.categories.toArray(), db.budgets.toArray(), db.accounts.toArray(),
    ]);
    return { businesses, inventoryItems, sales, customers, businessExpenses, personalIncomes, personalExpenses, categories, budgets, accounts };
  }
  ...
  async replaceData(data: BackupData['data']): Promise<void> {
    await db.transaction('rw', [
      ...
      db.personalIncomes, db.personalExpenses, db.categories, db.budgets, db.accounts,
    ], async () => {
      await Promise.all([
        ...
        db.categories.clear(), db.budgets.clear(), db.accounts.clear(),
      ]);
      await Promise.all([
        ...
        db.budgets.bulkAdd(data.budgets), db.accounts.bulkAdd(data.accounts),
      ]);
    });
  }
}
